...
const profiles = [
    {
        ...
        links: [{ content: "https://alice.example.com", type: 'LINKEDIN' }],
    ...
    },
    {
        ...
        links: [{ content: "https://bob.example.com", type: 'LINKEDIN' }],
    ...
    },
    {
        ...
        links: [{ content: "https://carol.example.com", type: 'LINKEDIN' }],
        skills: ["Java", "Spring"],
...
async function runSeed() {
    ...
    for (const item of profiles) {
        await db.orm.public.Profile.create({
            ...
            links: (p) => p.create(item.links.map(link => ({
                content: link.content,
                type: link.type,
            }))),
            skills: (p) => p.create(item.skills.map(skill => ({
                content: skill
            }))),
            projects: (p) => p.create(item.projects.map(project => ({
                name: project.name,
                link: project.link,
            }))),
            workplaces: (p) => p.create(item.workplaces.map(workplace => ({
                name: workplace.name,
                role: workplace.role,
                from: workplace.from,
                to: workplace.to,
